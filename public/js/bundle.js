...
var displayMap = function displayMap(locations) {
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); //Extends the map bounds
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = type === 'data' ? 'http://127.0.0.1:8000/api/v1/users/updateMe' : 'http://127.0.0.1:8000/api/v1/users/updateMyPassword';
            _context.next = 4;
            return (0, _axios.default)({
              method: 'PATCH',
              url: url,
...
var stripe = Stripe('pk_test_7guCXvrcd5AsJD6KWgwWSyEB00U3lyfSta');
...
if (userDataForm) {
  userDataForm.addEventListener('submit', function (e) {
    e.preventDefault();
    var form = new FormData(); // Recreate multi part form data
